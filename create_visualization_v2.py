    ...
    <script>
        ...
        clusterIds.forEach(clusterId => {
            ...
            const hoverText = clusterData.map(d => {
                ...
                // Limit to ~220 chars (about 5 wrapped lines) for tooltip display
                const displayDesc = d.description.length > 220
                    ? d.description.substring(0, 220) + '...'
                    : d.description;

                return `<b>${d.name}</b><br>` +
                    `<i>${d.category}</i><br><br>` +
                    `${wrapText(displayDesc, 45)}`;
            });
