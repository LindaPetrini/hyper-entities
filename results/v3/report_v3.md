...
**Who's building toward this.** **RISC Zero** provides the zero-knowledge virtual machine for proof generation. **Chainlink** and **API3** supply decentralized oracle infrastructure. **Kleros** has operated a live jury-DAO since 2018, handling 1,000+ cases. Slashing mechanisms are operational in Ethereum proof-of-stake. The gap is not that nobody has the pieces but that nobody has tried to combine ZK-IoT with AI anomaly detection under DAO adjudication — the trust boundaries between these systems are undefined. TRL: 4; ZK-IoT proofs demonstrated on ESP32 microcontrollers at ~700ms.
